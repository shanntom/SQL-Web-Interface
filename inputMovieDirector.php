...
<body>
<h1>Add a Moive/Director Relation</h1>
...
<p>Director
<SELECT NAME="did" SIZE=1><option value="NULL"></option>
<?php 
	$db_connection = mysql_connect("localhost", "cs143", "");
		if (!$db_connection) {
	    	die('Could not connect: ' . mysql_error());
	...
	mysql_select_db("CS143", $db_connection);
	$query = "SELECT id, first, last, dob FROM Director ORDER BY last";
	$queryResult = mysql_query($query, $db_connection);
		if (!$queryResult) {
		    echo "Could not successfully run query ".$queryResult."from DB: ".mysql_error();
		    exit;
...
if (isset($_GET['Submit'])) {
	...
	if($_GET["did"] == "NULL") {
		echo "You must select an director</br>";
		$validInput = false;
	}
	if (! $validInput) {
	...
	//format query
	$queryFormat = "INSERT INTO MovieDirector VALUES (%d, %d)";
